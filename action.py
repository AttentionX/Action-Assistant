...
You can issue these commands:
	...
	REQEUESTINFO - request for clarification or more information from the user to help you achieve your objective
...
<link id=1>About</link>
<link id=2>Store</link>
<link id=3>Supermario Brothers</link>
<link id=4>Guardians of the Galaxy 3</link>
<link id=5>John Wick 4</link>
<link id=6>Sign in</link>
<img id=7 alt="(CGV)"/>
<input id=8 alt="Search"></input>
<button id=9>(Search by voice)</button>
...
<text id=18>Settings</text>
------------------
OBJECTIVE: Buy a movie ticket for 2 for this friday at 7pm
CURRENT URL: https://www.cgv.co.kr/ticket
YOUR COMMAND: 
REQEUESTINFO Which movie would you like to watch? Here are the options: Supermario Brothers, Guardians of the Galaxy 3, John Wick 4
'''
...
def getWebsite(command):
    ...
    Examples: 
    ==========
    1. Objective: "Buy a shirt from Amazon for under $20"
    1. Website: https://www.amazon.com/

    2. Objective: "Find a 2 bedroom house for sale in Anchorage AK for under $750k"
    2. Website: https://www.redfin.com/
    """
	prompt = f'{instruction}\n Objective: "{command}"\n Website: '
	return state.chatGPT(prompt)
...
if (
	...
	def get_gpt_command(objective, url, previous_command, browser_content):
		...
		prompt = f'{basic_prompt} {examples} {current_template}'
		...
		# if len(prompt) > 5000:
		# 	prompt = prompt[:5000]
		response = state.chatGPT(prompt, 'You are a helpful assistant.')
		return response
	...
	website = getWebsite(objective)
	print(website)
    
	_crawler.go_to_page(website)
	try:
		while True:
			browser_content = "\n".join(_crawler.crawl())
			prev_cmd = gpt_cmd
